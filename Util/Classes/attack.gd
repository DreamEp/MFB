...
func crit(crit_chance, increase_crit_damage) -> float:
	if can_crit:
		can_crit = false
		var num = randf_range(0,1)
		if num <= crit_chance:
			can_crit = true
			return increase_crit_damage
		else:
			can_crit = false
			return 1.0
	else:
...
func calculate_effective_damage(elemental_type, attack_damage) -> float:
	if attacker is Player:
		effective_damage = attack_damage * defense(attack_damage  * (attacker.increase_attack_damage / 100)) * crit(attacker.crit_chance * (attacker.increase_crit_chance / 100), attacker.increase_crit_damage / 100) * randomness() \
