...
@onready var parent_size = item_container.size # Size of the parent container
@onready var child_size = item_texture.size  # Original size of the child image
	
func _ready():
	if upgrade != null:
		item_texture.texture = load(upgrade["icon"])
		label_item_number.text = str(upgrade["level"])
		scaleChild(parent_size, child_size)

func update_level(key, update_upgrade):
